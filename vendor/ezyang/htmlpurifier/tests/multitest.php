 ...
 *   - file (f), xml, type: these correspond to the parameters in IndexController.php
 *
 * @note
...
foreach ($versions_to_test as $version) {
    ...
    if ($AC['type'] !== 'phpt') {
        ...
        switch ($AC['distro']) {
            ...
            case 'normal':
                $test->add(
                    new CliTestCase(
                        "$phpv $version IndexController.php --xml $flush_arg $type_arg --disable-phpt $file_arg",
                        $AC['quiet'], $size
                    )
                );
                if ($break) break;
            case 'standalone':
                $test->add(
                    new CliTestCase(
                        "$phpv $version IndexController.php --xml $flush_arg $type_arg --standalone --disable-phpt $file_arg",
                        $AC['quiet'], $size
                    )
                );
                if ($break) break;
    ...
    if (!$AC['disable-phpt'] && (!$AC['type'] || $AC['type'] == 'phpt')) {
        $test->add(
            new CliTestCase(
                $AC['php'] . " IndexController.php --xml --php \"$phpv $version\" --type=phpt",
                $AC['quiet'], $size
            )
        );
    }
